...
def get_mask(cfg, conflict_thresh=0.0, strict=False, save_mask=False):
    """
    ...
    for k, v in pred_data.items():
        ...
        if not strict:
            # min_conflict = 1.0
            # for i in v:
            #     cft_val = conflict[(i, gt_label_ind)]
            #     if cft_val < min_conflict:
            #         min_conflict = cft_val
...
def trans_pred(cfg: Config):
    with open(cfg.ill_img_data_path, 'rb') as f:
        val_data: list = pickle.load(f)  # [ent, label, img_name, label_idx]
    gt_info = {item[2]: [item[3], item[0], item[1]] for item in val_data}
    ...
    id2type = dict()
    for k, v in pred_data.items():
        gt_label_idx, ent = gt_info[k][:2]
        id2type[ent2id[ent]] = (v[:top_k], gt_label_idx)

    with open(cfg.id2type_path, 'wb') as f:
        pickle.dump(id2type, f)


if __name__ == "__main__":
    split = 'zh_en'  # 'fr_en' or 'ja_en' or 'zh_en'
    cfg = Config(split)
    build_conflict_matrix(cfg)
    #get_mask(cfg, conflict_thresh=1.0, strict=False, save_mask=False)
    #trans_pred(cfg)